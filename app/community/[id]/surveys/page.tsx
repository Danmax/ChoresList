          ...
          <div className="grid gap-4 md:grid-cols-2">
            {surveys.map((survey) => (
              <article key={survey.id} className="rounded-3xl bg-white p-5 shadow-sm">
                ...
                <div className="mt-4 flex flex-wrap gap-2"><Link href={`/community/${groupId}/surveys/${survey.id}`} className="rounded-xl bg-violet-500 px-4 py-2 text-sm font-black text-white">{survey.status === "draft" ? "Review" : survey.hasSubmitted ? "View" : "Take Survey"}</Link>{canManage && survey._count.submissions === 0 && <Link href={`/community/${groupId}/surveys/${survey.id}/edit`} className="rounded-xl bg-white px-4 py-2 text-sm font-black text-violet-600 ring-1 ring-violet-100">Edit</Link>}{canManage && survey.status !== "draft" && <Link href={`/community/${groupId}/surveys/${survey.id}/report`} className="inline-flex items-center gap-1 rounded-xl bg-slate-100 px-4 py-2 text-sm font-black text-slate-600"><BarChart3 size={15} /> Report</Link>}</div>
              </article>
            ))}
