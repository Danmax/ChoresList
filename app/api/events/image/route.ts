...
export async function POST(req: NextRequest) {
  try {
    ...
    const eventDir = path.resolve(UPLOADS_ROOT, "events", String(householdId));
    if (!eventDir.startsWith(UPLOADS_ROOT + path.sep)) {
      return NextResponse.json({ error: "Invalid destination" }, { status: 400 });
    }
    await mkdir(eventDir, { recursive: true });

    const filename = `${Date.now()}-event-${randomBytes(6).toString("hex")}.webp`;
    ...
    const relativePath = `/uploads/events/${householdId}/${filename}`;
    ...
    console.error("[API event image]", message);
