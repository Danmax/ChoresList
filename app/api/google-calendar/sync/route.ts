...
import { syncAllGoogleCalendars } from "@/lib/google-calendar";
...
export const POST = withErrors(async (req: NextRequest) => {
  ...
  const result = await syncAllGoogleCalendars(householdId);
  if (result.skipped) {
  ...
  if (result.failed > 0) {
    return NextResponse.json(
      { ...result, error: `${result.failed} calendar event${result.failed === 1 ? "" : "s"} failed to sync` },
      { status: 502 }
    );
  }
