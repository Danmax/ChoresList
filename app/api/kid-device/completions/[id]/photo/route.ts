...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    const completion = await prisma.taskCompletion.findFirst({
      ...
      include: {
        assignment: { select: { chore: { select: { requiresPhoto: true } } } },
      },
    });
    if (!completion) return NextResponse.json({ error: "Completion not found" }, { status: 404 });
    if (!completion.assignment.chore.requiresPhoto) {
      return NextResponse.json({ error: "This task does not require photos" }, { status: 400 });
    }
    ...
    const updated = await prisma.taskCompletion.update({
      where: { id: completion.id, householdId: session.householdId },
    ...
    console.error("[API kid-device photo]", message);
